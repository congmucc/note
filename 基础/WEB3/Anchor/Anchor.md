...
在 Solana 和 Anchor 开发中，签名的使用依赖于账户的权利和操作的要求。签名者用于授权交易中的特定操作，确保操作的合法性。以下是一些常见的需要签名的情境：
1. **创建账户（`init` 操作）**
...
```
> 需要在`genesis`文件夹下运行这段代码之后会开启一启动本地的 Solana 测试验证器，并会将 Metaplex Token Metadata 程序部署到 Localnet。
> 
