...
- **offset**：偏移量，随着记录在repl_baklog中的数据增多而逐渐增大。slave完成同步时也会记录当前同步的offset。如果slave的offset小于master的offset，说明slave数据落后于master，需要更新。（**每更新一次就增加一个偏移量，可以理解偏移量就是更新多少次**）
...
- 然后判断slave节点的slave-priority值，越小**优先级**越高，如果是0则永不参与选举
