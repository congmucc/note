...
fn get_str() -> &String {
    let s = String::from("hello");
    &s // ❌ 错误：s 在函数结束时被释放，返回它的引用是悬垂引用
}
```
> 原因： s 是这个函数内部创建的，当函数返回后就被销毁，但你却试图返回对它的引用，会造成 悬垂引用（Dangling Reference）。


编译器在没有显式注解的情况下，使用三个规则来推断这些生命周期：

1. 第一个规则是，每个引用参数都有自己的生命周期参数
2. 第二个规则是，若只有一个引用参数，则它就是返回值的生命周期
3. 第三个规则是，若有多个引用参数且其中一个是 `&self`，则返回值生命周期与 `self` 相同。因为在这种情况下它是一个方法，所以seIf 的生命周期被分配给所有输出生命参数
```rust
// 不需要写 'a，编译器会自动加上
...
`// 不需要写 'a，编译器会自动加上 fn name_len(name: &str) -> usize {     name.len() }`
