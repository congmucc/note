...
> 在 Solana 中，账户内存是在创建时通过 `create_account` 类指令手动申请的，并由 `lamports` 预付租金（或设置 rent-exempt），但释放只能通过系统程序销毁账户，否则账户及其内存永久保留。

> Solana **不会自动释放内存**，释放逻辑必须手动触发！


**释放的方式**是：将账户 lamports 转给别人，然后将其 data 清空，owner 设为 `SystemProgram`
```
    #[account(
        mut,
        seeds = [
...
| 维度   | 租金不足                                                             | 手动删除账户             |
| ---- | ---------------------------------------------------------------- | ------------------ |
| 是否主动 | 否，系统周期性自动收租                                                      | 是，程序或用户主动调用关闭      |
| 数据状态 | 数据仍存在，但可被回收                                                      | 数据立即清空，账户关闭        |
| 余额处理 | lamports 被周期性扣减，最终余额可能为零，如果余额扣光，账户数据会被标记为可重用（垃圾回收），账户“失效”，数据会丢失。 | 余额一次性转移给指定账户       |
| 资源释放 | 延迟释放，依赖网络租金收取机制                                                  | 立即释放，避免无用账户占用存储和租金 |
| 可控性  | 受系统规则限制，用户无法直接控制                                                 | 用户/程序可随时控制关闭账户     |
