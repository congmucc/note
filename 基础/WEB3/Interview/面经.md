...
|---|---|---|
|**Bubblegum（泡泡糖）**|压缩 NFT（Compressed NFTs），降低存储成本|适用于游戏资产、大规模 NFT 发行|
|**Core（核心）**|新一代 NFT 标准|提供更灵活的 NFT 结构|
|**Fusion（融合）**|NFT 嵌套 NFT|角色装备系统、动态 NFT 组合|
|**Inscription（铭刻）**|Solana 上的 NFT 刻写|永久存储数据，类似比特币 Ordinals|
|**Token Metadata（代币元数据）**|NFT 元数据标准|NFT 交易市场、艺术品收藏|
|**Candy Machine（糖果机）**|NFT 发行工具（Launchpad）|公平铸造（Fair Mint）、白名单销售|
|**Core Candy Machine（核心糖果机）**|数字资产发行工具|适用于所有代币化资产|
|**Token Auth Rules（代币授权规则）**|NFT 权限管理|仅特定地址可转移、访问受限 NFT|
|**Hydra（九头蛇）**|Fanout 钱包（收益分配）|DAO 分红、NFT 版税|
|**MPL-Hybrid（混合资产）**|混合资产（NFT + FT）|半同质化代币（SFT）、游戏道具、门票|
