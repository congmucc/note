...
智能合约中，函数选择器（selector）是函数签名的哈希的前4个字节。例如`mint(address account)`的选择器为`bytes4(keccak256("mint(address)"))`，也就是`0x6a627842`。更多关于选择器的内容见[4.13 函数选择器Selector](# 4.13 函数选择器Selector)
...
这里的代理合约和可升级合约的非常相近，只是`fallback()`函数限制了管理员地址的调用。
...
这里的新、旧逻辑合约与**可升级合约**一样。逻辑合约包含`3`个状态变量，与保持代理合约一致，防止插槽冲突；包含一个函数`foo()`，旧逻辑合约会将`words`的值改为`"old"`，新的会改为`"new"`。
...
我们在透明代理已经学习了"选择器冲突"（Selector Clash），即合约存在两个选择器相同的函数，可能会造成严重后果。作为透明代理的替代方案，UUPS也能解决这一问题。
...
首先我们要复习一下[4.7 Delegatecall](# 4.7 Delegatecall)。如果用户A通过合约B（代理合约）去`delegatecall`合约C（逻辑合约），上下文仍是合约B的上下文，`msg.sender`仍是用户A而不是合约B。因此，UUPS合约可以将升级函数放在逻辑合约中，并检查调用者是否为管理员。

>  **注意：**复习一下Delegatecall，如何修改代理合约的逻辑地址是涉及到上下文的知识，这个主要是有一个共享状态变量的关系。
>
> 场景：
>
> 逻辑合约和代理合约的`    address public implementation; // 逻辑合约地址`是共享的状态变量，如果admin通过逻辑合约修改逻辑合约的地址，实际上是在代理合约的上下文中执行逻辑合约的代码，包括对共享状态变量的访问和修改。
>
> 这里也就为什么可以在逻辑合约修改之后代理合约不用改变了，因为由于上下文的原因修改过了。
...
UUPS的逻辑合约与**可升级合约**的不同是多了个升级函数。UUPS逻辑合约包含`3`个状态变量，与保持代理合约一致，防止插槽冲突。它包含`2`个
...
## 5.21 ERC4626 代币化金库标准

ERC4626 扩展了 ERC20 代币标准，旨在推动收益金库的标准化。这一讲，我们将介绍 DeFi 新一代标准 ERC4626，并写一个简单的金库合约。教学代码参考 openzeppelin 和 solmate 中的 ERC4626 合约，仅用作教学。
...
## 5.23 ERC-2612 ERC20Permit

我们之前介绍了ERC20，以太坊最流行的代币标准。它流行的一个主要原因是 `approve` 和 `transferFrom` 两个函数搭配使用，使得代币不仅可以在外部拥有账户（EOA）之间转移，还可以被其他合约使用。
...
[![img](assets/53-1.png)](https://github.com/AmazingAng/WTF-Solidity/blob/main/53_ERC20Permit/img/53-1.png)

### 5.23.2 合约



#### 5.23.2.1 IERC20Permit 接口合约
...
接下来，我们以可乐（$COLA）和美元（$USD）的市场为例，给大家介绍 AMM。为了方便，我们规定一下符号: 𝑥 和 𝑦 分别表示市场中可乐和美元的总量， Δ𝑥 和 Δ𝑦 分别表示一笔交易中可乐和美元的变化量，$L$ 和 Δ𝐿 表示总流动性和流动性的变化量。
...
其中 𝑘 为常数。也就是说，在交易前后市场中可乐和美元数量的总和保持不变。举个例子，市场中流动性有 10 瓶可乐和 10 美元，此时 𝑘=20，可乐的价格为 1 美元/瓶。我很渴，想拿出 2 美元来换可乐。交易后市场中的美元总量变为 12，根据约束$k=20$，交易后市场中有 8 瓶可乐，价格为 1 美元/瓶。我在交易中得到了 2 瓶可乐，价格为 1 美元/瓶。
...
其中 𝑘 为常数。也就是说，在交易前后市场中可乐和美元数量的乘积保持不变。同样的例子，市场中流动性有 10 瓶可乐和 10 美元，此时 𝑘=100，可乐的价格为 1 美元/瓶。我很渴，想拿出 10 美元来换可乐。如果在 CSAMM 中，我的交易会换来 10 瓶可乐，并耗尽市场上可乐的流动性。但在 CPAMM 中，交易后市场中的美元总量变为 20，根据约束 𝑘=100，交易后市场中有 5 瓶可乐，价格为 20/5=4 美元/瓶。我在交易中得到了 5 瓶可乐，价格为 10/5=2 美元/瓶。
...
1. 代币池被首次添加流动性时，LP份额 Δ𝐿 由添加代币数量乘积的平方根决定:
...
接下来，我们需要实现移除流动性的功能。当用户从池子中移除流动性 Δ𝐿 时，合约要销毁LP份额代币，并按比例将代币返还给用户。返还代币的计算公式如下:
...
在Swap合约中，用户可以使用一种代币交易另一种。那么我用 Δ𝑥单位的 token0，可以交换多少单位的 token1 呢？下面我们来简单推导一下。
...
交易前后 𝑘 值不变，联立上面等式，可以得到：

Δ𝑦=−Δ𝑥∗𝑦/𝑥+Δ𝑥

因此，可以交换到的代币数量 Δ𝑦 由 Δ𝑥，$x$，和 𝑦 决定。注意 Δ𝑥 和 Δ𝑦 的符号相反，因为转入会增加代币储备量，而转出会减少。
...
- `struct + memory`
- 使用链下事件，像**the graph**
- 显示数据别在链上搞，链上只是存，显示交给链下来做”。


**下面是`struct + memory`**

但需要注意数组过大会爆 ABI 编码限制（比如数组长度 >1000 时返回数据可能超出限制）。
...
那么，我们如何在不使用 for 循环的情况下从数组中检索所有数据呢？其实很简单。 **我们的方式** 首先，创建一个结构，其中包含您要检索的数据类型的数组。在这个例子中，我们将使用地址：
...
在返回值中，您将获得一个地址元组。不需要 for 循环！这种方法可以**节省 gas 成本**并使您的代码更高效。
...
- 在 Solidity 中，当你**返回一个包含动态数组的 struct（结构体）**时，Solidity 编译器**会自动生成 ABI 编码逻辑**，**将整个数组一起打包返回**。
- 因为你返回的是整个 `Addresses memory` 结构体，其中包含一个 `address[]`，它会自动将所有数组数据返回。
- 换句话说，**ABI 编码器会在调用返回值时把整个数组装进去返回**，就好像在 for 循环里填充返回数组一样，只是这个过程是自动完成的。
- 降低了gas
...
当然，以下是对 **Raydium**、**Balancer**、**Curve** 这三个 DeFi 协议的简明解释，适合面试或技术文档使用：

---

## **1. Raydium（基于 Solana 的 AMM 与流动性平台）**

^620b0f

- **链平台**：Solana
- **核心机制**：AMM（自动做市商）+ 订单簿流动性整合
- **特点**：
    - 支持类似 Uniswap 的 AMM 自动撮合，同时接入 **Serum** 的订单簿，融合 AMM 与订单簿深度。
    - 用户既可通过 AMM 流动池交易，也可享受订单簿撮合带来的更优价格。
    - 提供流动性挖矿、LP 奖励、交易挖矿等功能。

---

## **2. Balancer（自定义权重的 AMM 协议）**

- **链平台**：Ethereum（兼容 L2，如Polygon、Arbitrum）
- **核心机制**：多资产池 + 自定义权重 AMM
- **特点**：
    - 可创建含有 2~8 种代币的池子，并支持 **非对称权重**（如 80%/20%、50%/25%/25%）。
    - 池子本质上是一个指数基金，允许任意比例加入/退出。
    - 可设置交易费用范围，支持 **自定义 AMM 策略（如自平衡）**。
    - 提供 Smart Order Router (SOR) 进行最优路径计算。
...
- **链平台**：Ethereum + 多链部署
- **核心机制**：稳定币交易的低滑点 AMM（使用 StableSwap 算法）
- **特点**：
    - 专为 **相似价格资产**（如 USDC/DAI、stETH/ETH）设计，优化低滑点与高资本效率。
    - 使用 **双曲线模型** 代替恒定乘积模型，实现更好的流动性密度。
    - 支持 veCRV 模型：锁仓 CRV 获得治理权与交易分成。
    - 被广泛集成于 DeFi 协议中（如 Convex、Yearn）作为稳定币交易核心模块。
...
| 协议       | 类型        | 特色功能                | 适用场景         |
| -------- | --------- | ------------------- | ------------ |
| Raydium  | AMM + 订单簿 | 集成 Serum，快速撮合，低成本交易 | Solana链资产交易  |
| Balancer | 多资产 AMM   | 自定义权重，多代币组合池        | 多资产管理 + LP盈利 |
| Curve    | 稳定币 AMM   | 低滑点，veToken经济模型     | 稳定币 & 锚定资产交易 |

如需更深入了解如各自的数学模型、激励机制或代码设计，也可以告诉我继续展开。
