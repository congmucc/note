...
- **桶（Bucket）**聚合：用来对文档做分组
  - TermAggregation：按照文档字段值分组，例如按照品牌值分组、按照国家分组（**不能分词**）
...
详细安装步骤可以参考IK分词器的**安装过程**。

[安装ik分词器](安装elasticsearch/安装elasticsearch.md)
...
1. 拼音分词器适合在创建倒排索引的时候使用，但不能在搜索的时候使用。

   > 这是因为创建的时候可以创建一个拼音索引，但是搜索的时候如果使用拼音索引遇见**同音字**容易出错。

   ![image-20240111191536873](assets/image-20240111191536873.png)
...
![image-20240113002715359](assets/image-20240113002715359.png)

> 将数据进行分片，然后将分片的数据进行备份并交给其他节点，如果有一个节点挂了，还是可以合成一个完整的数据。
